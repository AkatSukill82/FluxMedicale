...
export default function MyCareNetTable({ transactions, invoices, currentUser, patient, onRefresh }) {
  ...
  const enrichedTransactions = transactions.map(tx => {
    const invoice = invoices.find(inv => inv.transaction_id === tx.transaction_id);
    const totalAmount = invoice?.total_amount || tx.prestations?.reduce((sum, p) => sum + (p.montant || 0), 0) || 0;
    
    return {
      ...tx,
      invoice,
      totalAmount,
      paidAmount: tx.status === 'ACCEPTED' ? totalAmount : 0,
      remainingAmount: tx.status === 'ACCEPTED' ? 0 : totalAmount
    };
  });
      ...
      <Card>
        <CardContent className="p-12 text-center">
          ...
          <p className="text-slate-500">
            Aucune transaction MyCareNet pour ce patient.
          </p>
        </CardContent>
      </Card>
    );
