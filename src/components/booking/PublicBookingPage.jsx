...
/**
 * Les praticiens proposés à la réservation sont configurés au déploiement, via
 * VITE_PUBLIC_BOOKING_DOCTORS (JSON : [{"email":"…","name":"Dr …"}]).
 *
 * Ils ne sont volontairement plus obtenus depuis l'API publique : l'ancienne
 * action `get_doctors` exposait à tout visiteur les adresses e-mail de
 * connexion des comptes administrateurs du cabinet.
 */
function configuredDoctors() {
  try {
    const raw = import.meta.env.VITE_PUBLIC_BOOKING_DOCTORS;
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed)
      ? parsed.filter((d) => d && typeof d.email === 'string')
      : [];
  } catch {
    console.error('VITE_PUBLIC_BOOKING_DOCTORS: JSON invalide');
    return [];
  }
}
...
export default function BookingPublic() {
  ...
  const [doctors] = useState(configuredDoctors);
  ...
  // Cabinet mono-praticien : présélection, pas de choix à faire
  useEffect(() => {
    if (doctors.length === 1) setSelectedDoctor(doctors[0].email);
  }, [doctors]);

  // Charger les créneaux quand on sélectionne une date et un médecin
  useEffect(() => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // Un jeton Turnstile est à usage unique : il faut refaire la
      // vérification avant toute nouvelle tentative.
